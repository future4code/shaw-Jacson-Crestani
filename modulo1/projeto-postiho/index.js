...
// // Exercício 7 
 ...
 const avisoAosAtrasados = () => {    const usuarios = [        { nome: "Artur", imunizacao: "incompleta" },
        { nome: "Barbara", imunizacao: "completa" },
       { nome: "Carlos", imunizacao: "incompleta" }/     ]

     for(let i = 0; i < usuarios.length; i++){       console.log(`Olá ${usuarios[i].nome}! Sua imunização está ${usuarios[i].imunizacao}, por favor volte ao postinho para tomar a segunda dose.`)
         }
} 
...
const avisoAosAtrasadosDesafio = () => {
    ...
    usuarios.map((item) => {
        if (item.imunizacao === 'incompleta') {
            console.log(`olá ${item.nome}! Sua imunização está ${item.imunizacao}, por favor volte ao postinho para tomar a vacina.`)
        }
    })
}
